...
...               "return this.onbefore{event}.call(this, event, fromState, toState, ...args);}}").format(event=e, header=header))
...
...               "return this.onleave{event}.call(this, event, fromState, toState, ...args);}}").format(event=s, header=header))
...
...     for e in events:
...         print(("onbefore{event}(event, fromState, toState, ...args){{ return true; }}").format(event=e, header=header))
...         print(("onafter{event}(event, fromState, toState, ...args){{}}").format(event=e, header=header))
...     for s in states:
...         print(("onleave{event}(event, fromState, toState, ...args){{ return true; }}").format(event=s, header=header))
